import { createClient } from '@supabase/supabase-js';
const supabase = createClient(
  'https://dribchwasdoxumbvhoaf.supabase.co',
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRyaWJjaHdhc2RveHVtYnZob2FmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODczMTM1MDMsImV4cCI6MjEwMjg4OTUwM30.gvh4SgeoRFZ6TaxuRziBNlKzuI87i1CEZoBebvIWrfY'
);
async function run() {
  const { data: ings } = await supabase.from('ingredients').select('*');
  const salt = ings.find(i => i.name.toLowerCase().includes('salt') || i.name.toLowerCase().includes('ఉప్పు'));
  console.log("Salt:", salt);
  if (!salt) return;
  const { data: txns } = await supabase.from('stock_transactions').select('*').eq('ingredient_id', salt.id).order('created_at', { ascending: false }).limit(5);
  console.log("Txns:", txns);
}
run();

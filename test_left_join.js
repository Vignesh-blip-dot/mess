...
async function check() {
  const { data, error } = await supabase.from('stock_transactions').select('*, profiles(name)').is('created_by', null).limit(1);
  console.log('left join with null fk:', data?.length, error);
}
check();
